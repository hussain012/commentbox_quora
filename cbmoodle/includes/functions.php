...
class Wall_Updates {
    ...
    function Wall_Updates($w) {
     ...
     // Updates   	
	  public function Updates($uid, $order) 
	{
		...
		if($order == 0)
           ...
           $query = mysql_query("SELECT M.course_id, M.activity_id, M.msg_id, M.uid_fk, M.message, M.created, U.username, U.firstname, U.lastname FROM messages_activity M, mdl_m22_user U  WHERE M.uid_fk=U.id and M.course_id='$wcid' and M.activity_id='$id' order by M.msg_id desc ") or die(mysql_error());
		else
		   //Ascending order! Use this for normal numbered lists!  FIFO!
		   $query = mysql_query("SELECT M.course_id, M.activity_id, M.msg_id, M.uid_fk, M.message, M.created, U.username, U.firstname, U.lastname FROM messages_activity M, mdl_m22_user U  WHERE M.uid_fk=U.id and M.course_id='$wcid' and M.activity_id='$id' order by M.msg_id") or die(mysql_error());
	...
	//Comments
	   public function Comments($msg_id, $id) 
	{
	    $wcid = (int) $_GET['CourseId'];
		//$wcid = $this->cid;
		...
		$query = mysql_query("SELECT C.course_id, C.com_id, C.uid_fk, C.comment, C.created, U.username, U.firstname, U.lastname FROM comments_activity C, mdl_m22_user U WHERE C.uid_fk=U.id and C.msg_id_fk='$msg_id' and C.course_id='$wcid' order by C.com_id asc ") or die(mysql_error());
	   while($row=mysql_fetch_array($query))
	    $data[]=$row;
        if(!empty($data))
		{
	...
	{                             
	   ...
	   if(!empty($row))
	   {
		...
		$ustring = "select * from mdl_m22_user where id = '2'";
        $user = $DB->get_record_sql($ustring);
        $data = $OUTPUT->user_picture($user, array('size'=>136));
	...
	//Insert Update
	public function Insert_Update($w, $uid, $id, $update) 
	{
	$this->cid = $w;
	$update=htmlentities($update);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
        ...
        if ($update!=$result['message']) {
            ...
            $newquery = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, M.activity_id, U.username FROM messages_activity M, mdl_m22_user U where M.uid_fk=U.id and M.uid_fk='$uid' order by M.msg_id desc limit 1 ");
	...
	//Insert Comments
	public function Insert_Comment($w, $uid, $msg_id, $id, $comment) 
	{
	//$wcid = (int) $_GET['CourseId'];
	$this->cid = $w;
	$comment=htmlentities($comment);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
		...
		if ($comment!=$result['comment']) {
            ...
            $newquery = mysql_query("SELECT C.course_id, C.activity_id, C.com_id, C.uid_fk, C.comment, C.msg_id_fk, C.created, U.username FROM comments_activity C, mdl_m22_user U where C.uid_fk=U.id and C.uid_fk='$uid' and C.msg_id_fk='$msg_id' and C.course_id='$w' and C.activity_id='$id' order by C.com_id desc limit 1 ");
	...
	{
	    $query = mysql_query("DELETE FROM `comments_activity` WHERE uid_fk='$uid' and com_id='$com_id'") or die(mysql_error());
        return true;
